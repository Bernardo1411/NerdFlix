...
class SignUp extends Component {
  ...
  inputValueHandler = (event, inputName) => {
    const newFormControl = updateObject(this.state.formControl, {
      [inputName]: updateObject(this.state.formControl[inputName], {
        value: event.target.value,
        valid: checkValidity(
          event.target.value,
          this.state.formControl[inputName].validation
        ),
        touched: true,
      }),
    });
  ...
  submitAuthHandler = (event) => {
    ...
    this.props.auth(
      ...
      this.state.formControl.Name.value,
      this.state.formControl.location.value
    );
  };
  ...
  render() {
    ...
    return (
      <div className={classes.Login}>
        <h2>Sign Up Now!</h2>
        <form className={classes.Form} onSubmit={this.submitAuthHandler}>
          {form}
          <Button
            disabled={
              !(
                this.state.formControl.email.valid &&
                this.state.formControl.password.valid
              )
            }
          >
            SUBMIT
